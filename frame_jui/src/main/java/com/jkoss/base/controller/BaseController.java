	...
	public Object exception(Exception e, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (e instanceof BindException) {
			...
			for (FieldError error : bindException.getBindingResult().getFieldErrors()) {
				out.write(JSON.toJSONString(ajaxError(error.getDefaultMessage() == null ? Constant.DWZRESULT_MESSAGE_ERROR : error.getDefaultMessage())));
				out.flush();
				out.close();
				return null;
			}
		...
		if (isAjax(request) || request.getParameter("ajax") != null) {
			out.write(JSON.toJSONString(ajaxError(e.getMessage() == null ? Constant.DWZRESULT_MESSAGE_ERROR : e.getMessage())));
			out.flush();
			out.close();
			return null;
		}
